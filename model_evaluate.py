...
def pipeline(test):
    ...
    data_time = data_time[keys] 

    test['B11_end'] = test['B9_start'] + test['B9_B10_B11_interval']
    test.loc[test['B11_end']>24, 'B11_end'] = test.loc[test['B11_end']>24, 'B11_end'] - 24
    test['A20_end'] = test['A20_start'] + test['A20_interval']
    test.loc[test['A20_end']>24, 'A20_end'] = test.loc[test['A20_end']>24, 'A20_end'] - 24
    test['A28_end'] = test['A28_start'] + test['A28_interval']
    test.loc[test['A28_end']>24, 'A28_end'] = test.loc[test['A28_end']>24, 'A28_end'] - 24
    test['B4_end'] = test['B4_start'] + test['B4_interval']
    test.loc[test['B4_end']>24, 'B4_end'] = test.loc[test['B4_end']>24, 'B4_end'] - 24
    
    test['收率'] = 0
    test['样本id'] = 0
    data_time = pd.concat([data_time, test[keys]], axis=0)
    ...
    if not os.path.exists('./train_data/diff_train.lz4'):
        diff_tmp_list = Parallel(n_jobs=-1, verbose=0)(delayed(gen_new_train)(i) for i in range(1, train_len))
    ...
    diff_tmp_list = Parallel(n_jobs=10, verbose=1)(delayed(gen_new_test)(i)\
    ...
    sub_df = pd.DataFrame()
    sub_df['样本id'] = 'sample_0'
    sub_df['收率'] = test['pre_target_mean'].values
    sub_df['收率'] = sub_df['收率'].apply(lambda x:round(x, 3))
